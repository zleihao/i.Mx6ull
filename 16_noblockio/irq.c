...
static ssize_t chr_read(struct file* file, char __user* buf, size_t count, loff_t* ppos)
{
    ...
    if (file->f_flags & O_NONBLOCK) {  /* 非阻塞方式 */
        if (atomic_read(&dev->key_state) == 0) {
            return -EAGAIN;
        }
    } else {                           /* 阻塞方式 */
        /* 等待事件 */
        wait_event_interruptible(dev->r_wait, atomic_read(&dev->key_state));
    }
...
{
    int mask = 0;
    struct mx6ull_irq_device* dev = (struct mx6ull_irq_device*)filp->private_data;
...
static void timer_func(unsigned long arg) 
{
    ...
    if (atomic_read(&dev->key_value)) {
        wake_up(&dev->r_wait);
    }
}

static int keyio_init(struct mx6ull_irq_device *dev) 
{
    int err = 0;
    int i = 0;
...
//-------------------------------------------------------------------
    ...
    /* 初始化队列头 */
    init_waitqueue_head(&mx6ull_irq_dev.r_wait);
